...
def count_tribe_cells(grid):
    """Count occupied cells per tribe."""
    tribe_cells = {}
    for row in grid:
        for cell in row:
            if cell.tribe:
...
def smooth(data, window=50):
    """Apply simple moving average for smoother plots."""
...
def evaluate_all(rows, cols, num_tribes, num_episodes=1000, log_interval=100, save_interval=500):
    """
    Evaluate and compare performance of trained MAPPO, QMIX, Hi-MAPPO (with and without MCTS),
    and a random baseline agent in the Civilization environment.
    """
    ...
    goal_dim = 3
    steps_per_episode = 8
    mcts_interval = 20  # how often to use MCTS in Hi-MAPPO
    ...
    # initialize log containers for metrics
    logs = {key: [] for key in [
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for _ in range(steps_per_episode):
            ...
            if episode % mcts_interval == 0:
                goal_list = []
                for i in range(num_tribes):
                    mcts = MCTS(hi_agent, hi_env, state_tensor=state, num_simulations=10)
                    best_goal = mcts.run()
                    goal_list.append(best_goal)
                goals = torch.tensor(goal_list)
            else:
    ...
    axs[0, 0].plot(sm(logs['hi_pops']), label="Hi-MAPPO (MCTS)")
    ...
    axs[0, 1].plot(sm(logs['hi_foods']), label="Hi-MAPPO (MCTS)")
    ...
    axs[1, 0].plot(sm(logs['hi_scores']), label="Hi-MAPPO (MCTS)")
    ...
    axs[1, 1].plot(sm(logs['hi_cells']), label="Hi-MAPPO (MCTS)")
